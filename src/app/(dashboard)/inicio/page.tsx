...
export default function InicioPage() {
  const { user } = useUser();
  const { players, stats, isLoading } = useHome(); 
  const displayName = user?.username || user?.firstName || "Jugador";

  const conexiones = stats?.nuevas_conexiones || 0;
  const metaConexiones = 10;
  const porcentajeBarra = Math.min((conexiones / metaConexiones) * 100, 100);
  ...
  return (
    <div className="min-h-screen bg-[#0F0F11] text-white p-6 md:p-10">
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-12 mt-8">
            <div className="bg-[#161618] border-2 border-[#00C2FF] 
                ...
                <div className="relative z-10">
                  ...
                  <h2 className="text-7xl font-black text-[#00C2FF] mb-2 tracking-tighter">
                    {stats?.nuevas_conexiones || 0}
                  </h2>
                  ...
                  <p className="text-gray-500 text-sm">Jugadores que matchean contigo</p>
                    <div className="w-full bg-[#2A2A2D] h-2 rounded-full mt-8">
                        <div 
                          className="bg-[#00C2FF] h-full rounded-full shadow-[0_0_10px_#00C2FF] transition-all duration-1000 ease-out"
            ...
            <div className="flex flex-col gap-6">
                ...
                <div className="flex-1 bg-[#161618] border border-dashed border-[#FF6B00]/60 rounded-3xl p-6 flex items-center gap-3">
                    ...
                    <p className="text-gray-300 text-sm">
                        ...
                        Mantén tus horarios actualizados para mejores matches
                    </p>
                </div>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {players && players.length > 0 ? (
          ...
          <div className="col-span-3 text-center py-12 text-gray-500">
            No hay jugadores compatibles en este momento. Ajusta tu perfil para encontrar tu squad.
          </div>
        )}
      </section>
